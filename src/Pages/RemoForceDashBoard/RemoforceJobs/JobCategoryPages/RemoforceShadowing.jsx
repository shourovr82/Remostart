...
const RemoforceShadowing = () => {
  const { data: shadowingJobs } = useQuery(['shadowing'], () =>
    axios
      .get(`${process.env.REACT_APP_URL_STARTUP}/api/job/remoforce/shadowing`)
      .then((res) => res.data)
  );
    ...
    <RemoForceDashBoardItems>
      <section>
        ...
        {shadowingJobs?.length ? (
          ...
          //                 <p className="text-xs px-[5px] py-[5px] ">{skill}</p>
          //               </div>
          //             ))}
          //         </div>
          //         {/* salary  */}
          ...
          //   ))}
          // </section>
          <JobCard allJobsInfo={shadowingJobs} applyNowBtn={handleApplyNow} />
        ) : (
          <NoJob />
        )}
      </section>
